...
class DdicDataElementOutput(BaseModel):
	...
	typeKind: str = Field(default="", description="Type category of the DDIC data element, for example domain or predefinedAbapType.")
...
class DdicDataElementUpdateRequest(BaseModel):
	...
	typeKind: str | None = Field(None, description="New type category, for example domain or predefinedAbapType.")
	typeName: str | None = Field(None, description="New referenced type name, such as a domain name.")
	dataType: str | None = Field(None, description="New predefined ABAP data type, such as CHAR, NUMC, DEC, or DATS.")
	dataTypeLength: int | None = Field(None, description="New technical length for predefined ABAP types.")
	dataTypeDecimals: int | None = Field(None, description="New number of decimals for predefined ABAP types.")
...
def _build_dataelement_update_payload(current_xml: str, request: DdicDataElementUpdateRequest) -> str:
	...
	_set_if_provided(data_element, "dtel:dataType", request.dataType)
	_set_if_provided(data_element, "dtel:dataTypeLength", str(request.dataTypeLength) if request.dataTypeLength is not None else None)
	_set_if_provided(data_element, "dtel:dataTypeDecimals", str(request.dataTypeDecimals) if request.dataTypeDecimals is not None else None)
