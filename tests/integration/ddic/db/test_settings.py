...
from ddic.db.settings import (
    DdicTableDbSettingsUpdateRequest,
    call_ddic_table_db_settings_lock,
    call_ddic_table_db_settings_read,
    call_ddic_table_db_settings_unlock,
    call_ddic_table_db_settings_update,
)
...
def _update_db_settings(system_id: str, table_name: str, request: DdicTableDbSettingsUpdateRequest, transport_number: str = ""):
    lock_response = call_ddic_table_db_settings_lock(system_id, table_name)
    assert lock_response.result is True
    assert lock_response.data is not None
    try:
        update_response = call_ddic_table_db_settings_update(
            systemId=system_id,
        ...
        call_ddic_table_db_settings_unlock(system_id, table_name, lock_response.data.lockHandle)
    return update_response
...
def test_ddic_table_db_settings_tmp_builtin(clean_sap_session):
    ...
    try:
        assert call_ddic_table_create(
            systemId=system_id,
            name=table_name,
            description="Pytest local table for db settings",
            packageName="$TMP",
            language="EN",
            responsible="DEVELOPER",
        ).result is True
        assert _update_table(system_id, table_name, _table_source_builtin(table_name)).result is True

        read_response = call_ddic_table_db_settings_read(system_id, table_name)
        assert read_response.result is True
        assert read_response.data is not None
        assert read_response.data.dataClassCategory == "APPL0"

        update_response = _update_db_settings(
            system_id,
            table_name,
            DdicTableDbSettingsUpdateRequest(
        ...
        assert update_response.data.dataClassCategory == "APPL1"
        assert update_response.data.storageType == "C"
        assert update_response.data.sharingType == "L"
    finally:
        _delete_table(system_id, table_name)


@pytest.mark.integration
def test_ddic_table_db_settings_transport_dataelement(ddic_transport_package):
    ...
    try:
        assert call_ddic_domain_create(
            systemId=system_id,
            name=domain_name,
            description="Pytest transport db domain",
        ...
        assert call_ddic_dataelement_create(
            ...
            description="Pytest transport db data element",
            packageName=package_name,
            transportNumber=transport_number,
            language="EN",
            responsible="DEVELOPER",
        ...
        assert call_ddic_table_create(
            systemId=system_id,
            name=table_name,
            description="Pytest transport table for db settings",
            packageName=package_name,
            transportNumber=transport_number,
            language="EN",
            responsible="DEVELOPER",
        ...
        assert update_response.result is True
        assert update_response.data is not None
        assert update_response.data.tableName == table_name
        assert update_response.data.dataClassCategory == "APPL1"
        assert update_response.data.storageType == "C"
        assert update_response.data.sharingType == "L"
    finally:
